...
switch($controller){
    case 'login':
        ...
        if($_SERVER['REQUEST_METHOD']==='POST') $auth->login(); else $auth->loginForm();
        break;
    case 'logout':
        $auth = new AuthController(); $auth->logout(); break;
    case 'dashboard':
        if(isset($_SESSION['user']) && $_SESSION['user']['role']==='admin'){ $admin=new AdminController(); $admin->dashboard(); } else { $k=new KaryawanController(); $k->dashboard(); } break;
    case 'admin':
